...
| `LCD16x2`                 | `LCD16x2(byte i2cAddress)`                          | `SetCursor(int line, int characterPosition)`, `SetText(string text)`, `SetText(int line, int characterPosition, string text)`, `SetTextNoRefresh(string text)`, `Clear()`, `SlideText(string text, int line)` returns `LCDSlide` instance, `SlideShow(IEnumerable<string> lines, int line)` returns `LCDSlide` instance, `SlideShow(int line, params string[] lines)` returns `LCDSlide` instance                                                                                                                                                                                                              |
